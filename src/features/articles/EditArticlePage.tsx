...
export const EditArticlePage = () => {
  ...
  useEffect(() => {
    if (article) {
      reset({
        ...
        tagIds: article.tags.map((t) => t.id).join(', '),
      });
    }
  }, [article, reset]);
  ...
  const onSubmit = async (data: ArticleFormData) => {
    ...
    try {
      const tagIds = data.tagIds ? data.tagIds.split(',').map((tagId) => tagId.trim()) : [];
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <Button type="submit" isLoading={updateArticle.isPending}>
                Update Article
              </Button>
            </div>
          </form>
